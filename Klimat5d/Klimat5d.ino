...
#define minPos 1
#define maxPos 17
#define measureTempInterval 2000
...
unsigned long MainDelayMillis = 0, FastDelayMillis = 0, TempIntervalMillis=0;
...
void setup(void)
{
    ...
    if (Pause == 255)
        Pause = 10; //При первом запуске значение в ячейке будет 255, тогда устанавливаем значение по умолчанию
...
void loop(void)
{
    ...
    {
        if (KeyPressed && (millis() - KeyTimer) > 15000)
        { //Если клавиши не нажимались в течении 15сек. то записываем значения в EEPROM
...
void ReadKeyTemp() //Опрос клавиатуры
{
    ...
    if (ButtonC == HIGH)
        ...
        CountPress = 0;
    if (CountPress >= 5)
        setDelay(); //Если средняя клавиша была нажата в течении 5ти циклов то переходим к функции установки паузы
...
void setPos(int Pos) //Устанавливаем позицию
{
    ...
    if (Pos < minPos)
        Pos = minPos; //Если позиция меньше минимальной тогда устанавливаем минимальную
    if (Pos > maxPos)
        Pos = maxPos; //Если позиция больше максимальной тогда устанавливаем максимальную
    if (RememberPos == Pos)
        return; //Проверяем не пытаемся ли мы установить туже позицию что уже установлена

    servo.attach(2);
    delay(50);
    posReg = Pos; //Устанавливаем глобальную переменную
    RememberPos = Pos;
    switch (Pos) //Установка регулятора по заранее отградуированную позицию
    {
    case 1:
        servo.write(0);
        break;
